...
protocol Talkable {
  ...
  */
  ...
  // 메서드 요구
  func talk() // 프로토콜에서는 구현은 하지 않고 구현해야 하는 부분을 정의만 한다
...
protocol ReadSpeakable: Readable {
  // func read() 이 프로토콜을 따르게 되면 speak 뿐만 아니라 Readable의 read도 구현해주어야 한다
  func speak()
}

protocol ReadWriteSpeakable: Readable, Writeable{
  // func read() 이 프로토콜을 따르게 되면 speak 뿐만 아니라 Readable의 read도 구현해주어야 한다
  // func write() 이 프로토콜을 따르게 되면 speak 뿐만 아니라 Writeable의 write도 구현해주어야 한다
  func speak()
}
...
if let someReadSpeakable: ReadSpeakable = someAny as? ReadSpeakable {
  someReadSpeakable.speak()
} // someAny가 Superclass의 인스턴스 이므로 동작하지 않음
